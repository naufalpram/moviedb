...
import { dateFormatter, runtimeFormatter } from '../../../helper/dataFormatter';
import { FaStar } from 'react-icons/fa';

const BasicInfo = ({ result }) => {
  const { data } = result;
  const formattedDate = dateFormatter(data?.release_date || data?.first_air_date);
  const formattedRuntime = runtimeFormatter(data?.runtime);
  return (
    <div className='w-3/5 pr-16'>
        ...
        <div className='flex gap-5'>
          ...
          {data?.genres?.map(genre => <p className='bg-primary-400 py-1 px-2 text-center font-semibold'>{genre.name}</p>)}
